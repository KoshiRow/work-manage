class SuperDate extends Date {
	...
	static getYearMonthStrAfter(nMonth) {
		const clone = new SuperDate();
		clone.setMonth(clone.getMonth() + nMonth);
		return clone.getFullYear() + "年" + (clone.getMonth() + 1) + "月";
	}

	static getOneMonthDurationStrAfter(nMonth) {
		const sd = new SuperDate();
		if(nMonth == 0) {
			return sd.getFirstSuperDateAfter(0).toLocaleDateStr() + " ~ " + sd.toLocaleDateStr();
		} else {
			return sd.getFirstSuperDateAfter(nMonth).toLocaleDateStr() + " ~ " + sd.getLastSuperDateAfter(nMonth).toLocaleDateStr();
		}
	}

	getFirstSuperDateAfter(nmonth) {
		const clone = new SuperDate(this);
		clone.setMonth(clone.getMonth() + nmonth);
		clone.setDate(1);
		return clone;
	...
	getLastSuperDateAfter(nmonth) {
		const clone = new SuperDate(this);
		clone.setMonth(clone.getMonth() + nmonth + 1);
		clone.setDate(0);
		return clone;
	}
